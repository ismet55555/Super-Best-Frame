...
def _process_communication_get_thread():
    ...
    while not stop_thread_flag:
        ...
        if not response.ok:
            logging.error('[Slideshow Process] Failed to retrieve information from main process. Response text: {}'.format(response.text))
        else:
            logging.debug('[Slideshow Process] Successfully retrieved information from main process. Response text: {}'.format(response.text))
...
def slideshow_process(process_is_running):
    ...
    logging.info('[Slideshow Process] Mapping out all available image files in directory: {}'.format(imageDirectory))
    ...
    print('')
    logging.info('[Slideshow Process] Beginning picture frame image rotation. Use SPACEBAR to quit ...')
    try:
        # Main slideshow loop
        while process_is_running.value:
            ...
            if not process_communication_data['controls']['pause_continue']:
                ...
                logging.info('[Slideshow Process] Showing image "{}" (Index: {}, Size: {}x{})'.format(imageNames[img_end_index], img_end_index, imageFileObject_end.shape[1], imageFileObject_end.shape[0]))
                ...
                response = requests.post(url='http://localhost:5555/utility/process_communication_post', params=current_status)
                if not response.ok:
                    logging.error('[Slideshow Process] Failed to report current image information to main process. Response text: {}'.format(response.text))
                else:
                    logging.debug('[Slideshow Process] Successfully reportedcurrent image information to main process. Response text: {}'.format(response.text))
        ...
        logging.critical('[Slideshow Process] An error has occurred while displaying images. Exception: {}'.format(error))
